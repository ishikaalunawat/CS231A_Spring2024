...
def form_initial_voxels(xlim, ylim, zlim, num_voxels):
    ...
    side_length = np.cbrt((diff(xlim) * diff(ylim) * diff(zlim)) / num_voxels)
...
def carve(voxels, camera):
    ...
    voxels_homogen = np.hstack((voxels, np.ones((voxels.shape[0], 1))))
    voxels_2d = camera.P @ voxels_homogen.T
    voxels_2d = (voxels_2d / voxels_2d[-1, :])[:2, :].T.astype(int)

    valid_indices = (voxels_2d[:, 0] >= 0) & (voxels_2d[:, 0] < w) & (voxels_2d[:, 1] >= 0) & (voxels_2d[:, 1] < h)
    voxels_2d_valid = voxels_2d[valid_indices]
    voxels_valid = voxels[valid_indices]

    voxel_silhouette_indices = camera.silhouette[voxels_2d_valid[:, 1], voxels_2d_valid[:, 0]] > 0

    return voxels_valid[voxel_silhouette_indices]
