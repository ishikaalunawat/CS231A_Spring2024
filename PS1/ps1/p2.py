...
def compute_camera_matrix(real_XY, front_image, back_image):
    ...
    # get world coordinates from both images
    P_image1 = np.c_[real_XY, np.zeros([12,1]), np.ones((12,1))]
    P_image2 = np.c_[real_XY, np.full((12,1), 150), np.ones((12,1))]
    P = np.r_[P_image1, P_image2]

    # get image coordinates 
    p = np.r_[front_image, back_image]

    # solve for PM=p and append homogenous row
    M = np.linalg.lstsq(P, p, rcond = None)[0]
    M = np.r_[M.T, np.array([[0,0,0,1]])]
    return M
    # END YOUR CODE HERE
...
def rms_error(camera_matrix, real_XY, front_image, back_image):
    ...
    # project to image plane (XY) from camera matrix
    p_projected = P @ camera_matrix.T
    p_projected = p_projected[:,:2]

    # calculate rms error
    rms = np.sqrt(np.mean(np.linalg.norm(p - p_projected, axis=1) ** 2))
    return rms
    # END YOUR CODE HERE
